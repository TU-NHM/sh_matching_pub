...
file = user_dir / "clusters" / "clusters_2_90.uc"
tmp_file1 = user_dir / "clusters_out_2_90_nohit.txt"
tmp_file_nohits = user_dir / "iupac_out_vsearch.fasta"
tmp_cl_file = user_dir / "clusters" / name_folder / "tmp.txt"
tmp_singl_file = user_dir / "clusters" / name_folder / "singletons.txt"
...
with open(file) as f:
    ...
    for row in dataReader:
        if row[0] == "S":
            ...
            logging.info(f"CLP_F_90_NH\t{row[1]}")

logging.info(f"CLP_F_90_NH\tTotal no of sequences: {seq_counter}")
...
with open (tmp_cl_file, "w") as cl, open(tmp_singl_file, "w") as singl, open(tmp_file1) as f:
    ...
    for row in dataReader:
        ...
        if len(cluster_seqs) == 1:
            # singleton cluster
            singl_file = user_dir / "clusters" / name_folder / "singletons" / f"Singleton{row[0]}"
            ...
            cl_file = user_dir / "clusters" / name_folder / "clusters" / f"Cluster{row[0]}"
