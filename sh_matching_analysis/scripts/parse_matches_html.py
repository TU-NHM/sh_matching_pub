...
style_tag = "<style>\nbody {background-color: powderblue; padding: 5px; }\n\
...
th, td {padding: 2px; }\
tr:nth-child(even) {background-color: #CDEBEF;}\
...
with open(outfile, "w") as o, open(matches_file) as m:
    ...
    o.write("<table>\n<tr><th>Taxon name</th><th>#sequences</th><th>#SHs (1.5%)</th><th>1.5% SHs (sequence count)</th></tr>\n")
    ...
    for kgd in seq_taxonomy_dict:
        ...
        for phy in seq_taxonomy_dict[kgd]:
            ...
            for cls in seq_taxonomy_dict[kgd][phy]:
                tax_name_c = tax_name_p + ";" + cls
                o.write(f"<tr><td>&emsp;&emsp;{cls}</td><td>{seq_taxon_count[tax_name_c]}</td><td>{len(seq_taxon_sh_count[tax_name_c])}</td><td></td></tr>")
                for order in seq_taxonomy_dict[kgd][phy][cls]:
                    tax_name_o = tax_name_c + ";" + order
                    o.write(f"<tr><td>&emsp;&emsp; &emsp;{order}</td><td>{seq_taxon_count[tax_name_o]}</td><td>{len(seq_taxon_sh_count[tax_name_o])}</td><td></td></tr>")
                    for fam in seq_taxonomy_dict[kgd][phy][cls][order]:
                        ...
                        for gen in seq_taxonomy_dict[kgd][phy][cls][order][fam]:
                            ...
                            for spe in seq_taxonomy_dict[kgd][phy][cls][order][fam][gen]:
