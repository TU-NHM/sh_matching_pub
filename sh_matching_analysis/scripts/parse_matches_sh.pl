...
foreach my $threshold (@thresholds) {
    ...
    while (<MATCHES>) {
        ...
        if ($fields[2] eq "present") {
            ...
            # SH taxonomy
            if (defined($fields[3]) && defined($ucl_taxonomy_hash{$sh_ucl_hash{$fields[3]}})) {
                print MATCHES_OUT $ucl_taxonomy_hash{$sh_ucl_hash{$fields[3]}} . "\t";
                $duplicate_taxonomy = $ucl_taxonomy_hash{$sh_ucl_hash{$fields[3]}};
            } else {
                print MATCHES_OUT "\t";
            }
        ...
        if (!defined($fields[3])) {
            ...
            print MATCHES_OUT $sh_ucl_hash{$fields[3]} . "\t";
            $duplicate_ucl_code = $sh_ucl_hash{$fields[3]};
            # UCL taxonomy
            if (defined($ucl_taxonomy_hash{$sh_ucl_hash{$fields[3]}})) {
                print MATCHES_OUT $ucl_taxonomy_hash{$sh_ucl_hash{$fields[3]}} . "\t";
            } else {
                print MATCHES_OUT "\t";
            }
